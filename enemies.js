...
export function spawnEnemy(){
  ...
  const glyphs = { pawn:'♟', rook:'♜', bishop:'♝', queen:'♛' };
  const el = document.createElement('div');
  el.className = 'piece enemy';
  el.textContent = glyphs[type] + '\uFE0E';
...
function pickNextQueenTarget(e){
  const SIZE=8, cx=e.qx, cy=e.qy, ny=cy+1;
  const opts = [];
  for (var di=-1; di<=1; di++){
    const nx = cx+di; if (nx>=0 && nx<SIZE) opts.push(nx);
  }
  const nx = opts.length ? opts[Math.floor(Math.random()*opts.length)] : cx;
  e.qtx = nx; e.qty = ny;
}
function pickNextBishopTarget(e){
  const SIZE=8, cx=e.bx, cy=e.by;
  var nx = cx + e.bdir; const ny = cy + 1;
...
export function stepEnemies(dt){
  ...
  for (let i=state.enemies.length-1; i>=0; i--){
    ...
    if (e.type==='queen' || e.type==='bishop'){
      ...
      // Avoid optional chaining/nullish: compute the current target explicitly
      var tx, ty;
      if (e.type==='queen'){ tx = e.qtpX; ty = e.qtpY; }
      else                { tx = e.btpX; ty = e.btpY; }
      ...
      if (dist <= step || dist === 0){
        ...
        if (e.type==='queen'){
          ...
          if (e.qy >= 8){ e.el.remove(); state.enemies.splice(i,1); continue; }
          pickNextQueenTarget(e); e.qtpX = e.qtx*cell; e.qtpY = e.qty*cell;
        } else {
          e.bx = e.btx; e.by = e.bty;
          if (e.by >= 8){ e.el.remove(); state.enemies.splice(i,1); continue; }
      ...
      // pawns/rooks
      const vx = (e.vxCells||0)*cell*mult;
      const vy = (e.vyCells||0)*cell*mult;
      let newPx = e.px + vx*dtSec;
      if (newPx < 0) newPx = 0;
      if (newPx > maxX) newPx = maxX;
      e.px = newPx;
      e.py += vy*dtSec;
      e.el.style.left = e.px+'px'; e.el.style.top = e.py+'px';
    }
