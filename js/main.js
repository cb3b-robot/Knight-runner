...
const SIZE=8;
const CELL=()=> parseFloat(getComputedStyle(root).getPropertyValue('--cell')) || (game.clientWidth/8) || 64;
...
function unlockAudio(){
  if(audio.unlocked) return; ensureAudio();
  const s=audio.ctx.createBufferSource(); s.buffer=audio.ctx.createBuffer(1,1,22050); s.connect(audio.ctx.destination);
  try{s.start(0);}catch{}
  audio.unlocked=true; Music.start();
}
['pointerdown','keydown'].forEach(evt=>document.addEventListener(evt, unlockAudio, {once:true}));

function tone({freq=440,type='sine',dur=0.12,gain=0.05,attack=0.01,release=0.12,slideTo=null,slideTime=0.08}={}){
  if(!audio.enabled) return; ensureAudio();
  const t0=now(), osc=audio.ctx.createOscillator(), g=audio.ctx.createGain(), f=audio.ctx.createBiquadFilter();
  f.type='lowpass'; f.frequency.value=9000; osc.type=type; osc.frequency.value=freq;
  ...
  g.gain.setValueAtTime(gain,t0+dur); g.gain.linearRampToValueAtTime(0.0001,t0+dur+release);
...
const Music=(()=>{ let master,padGain,arpGain,delay,feedback,lowpass,timer=null,step=0,playing=false;
  function init(){ ensureAudio(); if(master) return;
    ...
    delay=audio.ctx.createDelay(1); delay.delayTime.value=0.22; feedback=audio.ctx.createGain(); feedback.gain.value=0.22;
    ...
    lowpass.connect(delay); delay.connect(feedback); feedback.connect(delay); delay.connect(master); lowpass.connect(master); master.connect(audio.ctx.destination);
  }
  function mtof(n){ return 440*Math.pow(2,(n-69)/12); }
  function note({freq,when,dur=0.22,type='triangle',gain=0.18}){ if(!audio.enabled) return;
    const o=audio.ctx.createOscillator(), g=audio.ctx.createGain(); o.type=type; o.frequency.value=freq;
    ...
    o.connect(g); g.connect(arpGain); o.start(when); o.stop(when+dur+0.02);
  }
  function pad({freq,when,dur=3.6,gain=0.10}){ if(!audio.enabled) return;
  ...
  function tick(){ if(!playing||!audio.enabled) return; const t=now(), bar=Math.floor(step/4), rootN=roots[bar%roots.length];
    if(step%4===0){ pad({freq:mtof(rootN),when:t+0.01}); pad({freq:mtof(rootN+7),when:t+0.01,gain:0.08}); }
    note({freq:mtof(rootN+arp[step%arp.length]),when:t+0.02}); step++; }
  ...
  function setEnabled(on){ master && (master.gain.value = on?0.16:0.0); }
...
applyMuteUI();
muteBtn.addEventListener('click', ()=>{ unlockAudio(); audio.enabled=!audio.enabled; try{localStorage.setItem(MUTE_KEY,String(!audio.enabled));}catch{} applyMuteUI(); });
...
function addScore(name,score){
  ...
  list.sort((a,b)=> b.score - a.score || a.ts - b.ts); const top=list.slice(0,50); saveScores(top); return top;
}
function renderLeaderboard(myName=null,myScore=null){
  lbList.innerHTML=''; const list=loadScores();
  if(list.length===0){ const li=document.createElement('li'); const a=document.createElement('div'); a.className='name'; a.textContent='No scores yet'; const b=document.createElement('div'); b.className='score'; b.textContent='—'; lbList.appendChild(li); lbList.appendChild(a); lbList.appendChild(b); return; }
  list.forEach((e,i)=>{ const li=document.createElement('li'); const name=document.createElement('div'); name.className='name'; name.textContent=`${i+1}. ${e.name}`; const sc=document.createElement('div'); sc.className='score'; sc.textContent=`${e.score.toFixed(1)}s`; if(myName && myScore!=null && e.name===myName && Math.abs(e.score-myScore)<1e-6){ name.style.color='#2ecc71'; sc.style.color='#2ecc71'; } lbList.appendChild(li); lbList.appendChild(name); lbList.appendChild(sc); });
}
renderLeaderboard();
resetBtn?.addEventListener('click', ()=>{ if(confirm('Clear all saved scores on this device?')){ localStorage.removeItem(LS_KEY); renderLeaderboard(); window.krFitBoard?.(); }});
toggleLB?.addEventListener('click', ()=>{
  ...
  window.krFitBoard?.(); // re-evaluate fit & scroll lock
});
...
function drawFirstArrow(dir){
  ...
  prim.setAttribute('class','ga-primary'+(insideB?'':' ga-invalid'));
  prim.setAttribute('marker-end', insideB?'url(#headPrimary)':'url(#headInvalid)');
  ...
  label.textContent = insideB ? 'then choose ⟂' : 'out of bounds';
  ...
  hints.forEach(h=>{ if(!inside(h.hx,h.hy)) return; const hx=h.hx*cell+cell/2, hy=h.hy*cell+cell/2;
...
function updateDots(){
  ...
  if(!running) return; const cell=CELL();
  knightOffsets.forEach(o=>{
    const tx=knight.x+o.x, ty=knight.y+o.y; if(!inside(tx,ty)) return;
...
// ---------- Power-ups ----------
let powerups=[]; const POWER_TYPES=['shield','speed','slow','clear']; const POWER_GLYPH={shield:'🛡',speed:'⚡',slow:'🕒',clear:'💥'};
function spawnPowerUp(){
  ...
  while(tries-- > 0){
    ...
    const el=document.createElement('div'); el.className='power'; el.style.left=`${x*CELL()}px`; el.style.top=`${y*CELL()}px`;
    el.innerHTML=`<div class="glyph">${POWER_GLYPH[type]}</div><div class="ring"></div>`;
    game.appendChild(el); const expiresAt=performance.now()+5500; powerups.push({el,type,x,y,expiresAt}); SFX.spawnPower(); break;
  }
}
function pickupAt(x,y){
  for(let i=powerups.length-1;i>=0;i--){ const p=powerups[i]; if(p.x===x && p.y===y){ applyPower(p.type,x,y); p.el.classList.add('fade'); setTimeout(()=>p.el.remove(),450); powerups.splice(i,1); } }
}
...
function sparkle(x,y){
  const cell=CELL();
  for(let i=0;i<6;i++){ const s=document.createElement('div'); s.className='sparkle';
    s.style.left=`${x*cell + cell*0.37 + Math.random()*cell*0.26}px`; s.style.top=`${y*cell + cell*0.37 + Math.random()*cell*0.26}px`;
    s.style.background = i%2? '#2ecc71':'#fff'; game.appendChild(s); setTimeout(()=>s.remove(),600); }
}

// ---------- Enemies ----------
let enemies=[]; const BASE_SPEED={ pawn:1.15, bishop:2.30, rook:3.20, queen:1.40 }; // cells/sec
function spawnEnemy(){
  const r=Math.random(); const type = (r>0.85)?'queen' : (r>0.65)?'rook' : (r>0.40)?'bishop' : 'pawn';
  const x=(Math.random()*SIZE)|0, y=-1; const el=document.createElement('div'); el.className='piece enemy';
  el.setAttribute('data-glyph', GLYPHS[type]); el.textContent=''; game.appendChild(el);
  const px=x*CELL(), py=y*CELL();
  if(type==='queen'){
    const e={el,type,px,py,qx:x,qy:y,qtx:x,qty:0,qtpX:x*CELL(),qtpY:0,stepSpeed:BASE_SPEED.queen};
    pickNextQueenTarget(e); e.qtpX=e.qtx*CELL(); e.qtpY=e.qty*CELL(); el.style.left=`${px}px`; el.style.top=`${py}px`; enemies.push(e); return;
  }
  if(type==='bishop'){
...
function pickNextQueenTarget(e){
  const cx=e.qx, cy=e.qy, ny=cy+1; const options=[]; for(const dx of [-1,0,1]){ const nx=cx+dx; if(nx>=0&&nx<SIZE) options.push(nx); }
  const nx=options.length ? options[(Math.random()*options.length)|0] : cx; e.qtx=nx; e.qty=ny;
}
function pickNextBishopTarget(e){
  const cx=e.bx, cy=e.by; let nx=cx+e.bdir; const ny=cy+1; if(nx<0||nx>=SIZE){ e.bdir=-e.bdir; nx=cx+e.bdir; }
  e.btx=nx; e.bty=ny;
}
...
function scheduleDifficulty(){
  difficultyTimer=setInterval(()=>{ if(!running) return; speedMult += 0.4; speedEl.textContent=`${(speedMult*(slowFactor||1)).toFixed(1)}×`; clearTimeout(spawnTimer); scheduleSpawn(); }, 6000);
}
...
function moveEnemiesSmooth(dt){
  ...
  for(let i=powerups.length-1;i>=0;i--){ if(nowT>powerups[i].expiresAt){ powerups[i].el.classList.add('fade'); setTimeout(()=>powerups[i].el.remove(),450); powerups.splice(i,1); } }
  ...
  for(let i=enemies.length-1;i>=0;i--){
    ...
    if(e.type==='queen'){
      ...
      if(dist<=step || dist===0){ e.px=e.qtpX; e.py=e.qtpY; e.el.style.left=`${e.px}px`; e.el.style.top=`${e.py}px`; e.qx=e.qtx; e.qy=e.qty;
        if(e.qy>=SIZE){ e.el.remove(); enemies.splice(i,1); continue; } pickNextQueenTarget(e); e.qtpX=e.qtx*CELL(); e.qtpY=e.qty*CELL();
      ...
      if(dist<=step || dist===0){ e.px=e.btpX; e.py=e.btpY; e.el.style.left=`${e.px}px`; e.el.style.top=`${e.py}px`; e.bx=e.btx; e.by=e.bty;
        if(e.by>=SIZE){ e.el.remove(); enemies.splice(i,1); continue; } pickNextBishopTarget(e); e.btpX=e.btx*CELL(); e.btpY=e.bty*CELL();
      } else { const ux=dx/dist, uy=dy/dist; e.px+=ux*step; e.py+=uy*step; e.el.style.left=`${e.px}px`; e.el.style.top=`${e.py}px`; }
    } else {
      const vx=(e.vxCells||0)*CELL()*mult, vy=(e.vyCells||0)*CELL()*mult; e.px=clampX(e.px + vx*dtSec); e.py+=vy*dtSec; e.el.style.left=`${e.px}px`; e.el.style.top=`${e.py}px`;
    }
    const ex=Math.round(e.px/CELL()), ey=Math.round(e.py/CELL());
    if(ex===knight.x && ey===knight.y){ if(shield>0){ shield=0; bShield.classList.remove('active'); e.el.remove(); enemies.splice(i,1); game.classList.add('shake'); setTimeout(()=>game.classList.remove('shake'),220); SFX.shieldHit(); } else { gameOver(); return; } }
    if(e.py>SIZE*CELL()){ e.el.remove(); enemies.splice(i,1); }
  }
  // Knight danger glow (any enemy at a knight move away)
  let danger=false;
  for(const o of knightOffsets){ const tx=knight.x+o.x, ty=knight.y+o.y; if(!inside(tx,ty)) continue; if(enemies.some(e=>Math.round(e.px/CELL())===tx && Math.round(e.py/CELL())===ty)){ danger=true; break; } }
...
// ---------- Knight Movement ----------
const knightOffsets=[{x:2,y:1},{x:2,y:-1},{x:-2,y:1},{x:-2,y:-1},{x:1,y:2},{x:1,y:-2},{x:-1,y:2},{x:-1,y:-2}];
function moveKnightTo(tx,ty){ if(!inside(tx,ty)) return; knight.x=tx; knight.y=ty; placeKnight(); SFX.jump(); pickupAt(tx,ty); checkCollision(); updateDots(); clearGuide(); if(speedMoves>0){ speedMoves--; if(speedMoves===0) bSpeed.classList.remove('active'); } }
...
function processArrow(dir){
  ...
  const dx2=firstArrow.x*2, dy2=firstArrow.y*2; let tx=knight.x, ty=knight.y;
...
document.addEventListener('keydown', (e)=>{ if(!running) return; const k=e.key.replace('Arrow','').toLowerCase(); const dir=DIRS[k]; if(!dir) return; e.preventDefault(); processArrow(dir); }, {passive:false});

// ---------- Game Loop ----------
function loop(t){ if(!running) return; const dt=t-lastTime; lastTime=t; moveEnemiesSmooth(dt); scoreEl.textContent=((t-startTime)/1000).toFixed(1); requestAnimationFrame(loop); }
...
function gameOver(){
  ...
  const over=document.createElement('div'); over.id='over'; const finalScore=parseFloat(scoreEl.textContent)||0;
  ...
  const nameInput=over.querySelector('#playerName'); const saveBtn=over.querySelector('#saveBtn'); const restartBtn=over.querySelector('#restart');
  const lastName=localStorage.getItem('knightRunner_lastName')||''; if(lastName) nameInput.value=lastName;
  saveBtn.addEventListener('click', ()=>{ const name=(nameInput.value||'Player').trim(); localStorage.setItem('knightRunner_lastName', name); addScore(name, finalScore); saveBtn.disabled=true; renderLeaderboard(name, finalScore); window.krFitBoard?.(); });
...
function restart(){
  ...
  speedEl.textContent='1.0×'; startTime=performance.now(); lastTime=startTime;
  ...
  SFX.restart(); Music.start(); window.krFitBoard?.();
}

// ---------- Start ----------
let dots=[]; updateDots(); scheduleSpawn(); scheduleDifficulty(); requestAnimationFrame(loop); applyMuteUI(); Music.setEnabled(audio.enabled);

// ---------- Resize sync ----------
const ro=new ResizeObserver(()=>{ const squares=document.querySelectorAll('.square'); squares.forEach((sq,i)=>{ const x=i%SIZE, y=(i/SIZE)|0; sq.style.left=`${x*CELL()}px`; sq.style.top=`${y*CELL()}px`; }); placeKnight(); guide.setAttribute('viewBox', `0 0 ${CELL()*SIZE} ${CELL()*SIZE}`); });
ro.observe(game);
