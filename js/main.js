// =========================================================
// Knight Runner — main.js (full file)
// =========================================================
...
const SIZE = 8;
const CELL = () =>
  parseFloat(getComputedStyle(root).getPropertyValue('--cell')) ||
  (game.clientWidth / 8) || 64;
...
function tone(opts){
  const {
    ...
    attack=0.01, release=0.12, slideTo=null, slideTime=0.08
  } = (opts||{});
...
const Music = (() => {
  ...
  function note({freq,when,dur=0.22,type='triangle',gain=0.18}){
  ...
  function pad({freq,when,dur=3.6,gain=0.10}){
    if (!audio.enabled) return;
  ...
  function tick(){
    ...
    const t=now(); const bar=Math.floor(step/4); const rootN=roots[bar%roots.length];
    if (step%4===0){ pad({freq:mtof(rootN),when:t+0.01}); pad({freq:mtof(rootN+7),when:t+0.01,gain:0.08}); }
    note({freq:mtof(rootN + pattern[step%pattern.length]), when:t+0.02});
    step++;
  }
  ...
  function setEnabled(on){ master && (master.gain.value = on ? 0.16 : 0); }
...
toggleLB?.addEventListener('click', ()=>{
  ...
  window.krFitBoard?.(); // Recompute fit/scroll lock
});
...
function drawFirstArrow(dir){
  ...
  const ex=clamp(tx*cell+cell/2, 0, (SIZE-1)*cell);
  const ey=clamp(ty*cell+cell/2, 0, (SIZE-1)*cell);
...
function spawnPowerUp(){
  ...
  while (tries-- > 0){
    ...
    // sanity guard:
    if (!powerups[powerups.length-1].el) {
      console.warn('[KR] Spawned power-up missing el!', powerups[powerups.length-1]);
    }
...
function pickupAt(x,y){
  for (let i=powerups.length-1; i>=0; i--){
    const p = powerups[i];
    if (!p || !p.el) { powerups.splice(i,1); continue; } // guard
    if (p.x===x && p.y===y){
      applyPower(p.type,x,y);
      p.el.classList.add('fade');
      setTimeout(()=> p.el && p.el.remove(), 450);
      powerups.splice(i,1);
    }
  }
}
...
function spawnEnemy(){
  ...
  el.setAttribute('data-glyph', GLYPHS[type]);
  el.textContent=''; game.appendChild(el);

  const px=x*CELL(), py=y*CELL();

  if (type==='queen'){
    // Queens choose forward-left/forward/forward-right each row
  ...
  if (type==='bishop'){
    // Bishops go down diagonally, bouncing horizontally at edges
  ...
  // Pawns & rooks: straight down (rooks faster)
...
function pickNextQueenTarget(e){
  ...
  for (const dx of [-1,0,1]){
    const nx=cx+dx; if (nx>=0 && nx<SIZE) options.push(nx);
  }
...
function moveEnemiesSmooth(dt){
  ...
  for (let i=enemies.length-1; i>=0; i--){
    ...
    if (e.type==='queen'){
      ...
      if (dist<=step || dist===0){
        ...
        const ux=dx/dist, uy=dy/dist; e.px+=ux*step; e.py+=uy*step;
        e.el.style.left=`${e.px}px`; e.el.style.top=`${e.py}px`;
      }
  ...
  // Knight danger glow: any enemy one knight move away?
  let danger=false;
...
// Keyboard two-step (2 in one axis, then 1 perpendicular)
...
document.addEventListener('keydown', (e)=>{
  ...
  e.preventDefault(); // avoid page scroll on Arrow keys
  processArrow(dir);
}, {passive:false});
...
function startGame(){
  ...
  applyMuteUI();
  Music.setEnabled(audio.enabled);
  window.krFitBoard?.(); // ensure initial fit after build
}
startGame();
...
function restart(){
  ...
  for (const p of powerups){ if (p && p.el) p.el.remove(); }
  powerups.length = 0;
